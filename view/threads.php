        ...
        <div class="DiscussionType">
            ...
            <?php foreach ($thread->messages as $km => $message) { ?>
                ...
                <?php if (empty($message->fragments)) { ?>
                    (suite à écrire..)
                <?php } else { ?>
                    <?php foreach ($message->fragments as $kf => $fragment) { ?><span style="text-decoration: white dotted underline"><?= $fragment->content ?></span> <?php } ?> <?php if (array_key_last($thread->messages) === $km) { ?>(suite à écrire..)<?php } ?>
                <?php } ?>
                </p>
            <?php } ?>
